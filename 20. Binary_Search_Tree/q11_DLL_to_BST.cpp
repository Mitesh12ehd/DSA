...
Node* convert_DLL_to_BST(Node* &head,int n){
    //base case
    if(n <= 0 || head == NULL){
        return NULL;
    }

    Node* leftSubTree = convert_DLL_to_BST(head,n-1-n/2);

    Node* root = head;
    root->left = leftSubTree;
    head = head->right;

    root->right = convert_DLL_to_BST(head,n/2);
    return root;
}
...
int main(){
    ...
    //convert DLL to BST
    Node* root1 = convert_DLL_to_BST(head,9);
    cout << endl;
    cout << "printing tree" << endl;
